...
import 'dart:async';
import 'dart:convert';
import '../service/basketball_service.dart';
...
class _BasketballPageState extends State<BasketballPage> {
  ...
  Future<void> _loadMarkersAndData() async {
    final jsonString = await rootBundle.loadString('assets/basketball.json');
...
// 대중교통 및 기차 추천 여부를 묻는 확인창
        ...
        content: Container(
          ...
          child: ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              return ListTile(
                title: Text(basketballClass['program']!),
                subtitle: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text("시설명: ${basketballClass['facility']}"),
                    Text("장소: ${basketballClass['place']}"),
                    Text("수강료: ${basketballClass['fee']}"),
                    Text("대상: ${basketballClass['target']}"),
                    Text("시간: ${basketballClass['time']}"),
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setState) => AlertDialog(
          ...
          content: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              Text("$programName ${isClassEnrollment ? '강좌' : '장소 이용'}을 위해 개인정보 동의가 필요합니다."),
              CheckboxListTile(
  ...
  void _showFormDialog(String programName, {required bool isClassEnrollment}) {
    ...
    showDialog(
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setState) => AlertDialog(
          title: Text("${isClassEnrollment ? '수강생' : '이용자'} 정보 입력"),
  ...
  void _showConfirmationDialog(bool isClassEnrollment) {
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        title: Text("${isClassEnrollment ? '강좌신청' : '장소신청'} 완료"),
        content: Text("${isClassEnrollment ? '강좌신청' : '장소이용 신청'}이 완료되었습니다."),
        actions: [
          TextButton(
            onPressed: () => Navigator.of(context).pop(),
            child: Text("확인"),
