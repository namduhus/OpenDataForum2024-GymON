...
import 'dart:async';
import 'dart:convert';
import '../service/baseball_service.dart';
...
class _BaseballPageState extends State<BaseballPage> {
  ...
  Future<void> _loadMarkersAndData() async {
    final jsonString = await rootBundle.loadString('assets/baseball.json');
...
// 오류 다이얼로그
  ...
  // 강좌 신청을 위한 다이얼로그
  Future<void> _showBaseballClasses() async {
    final baseballService = BaseballService();
    final classes = await baseballService.fetchBaseballClasses();
        ...
        content: Container(
          ...
          child: ListView.separated(
            ...
            itemBuilder: (context, index) {
              final baseballClass = classes[index];
              return ListTile(
                title: Text(baseballClass['program']!),
                subtitle: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text("시설명: ${baseballClass['facility']}"),
                    Text("장소: ${baseballClass['place']}"),
                    Text("수강료: ${baseballClass['fee']}"),
                    Text("대상: ${baseballClass['target']}"),
                    Text("시간: ${baseballClass['time']}"),
                    Text("기간: ${baseballClass['start']} ~ ${baseballClass['end']}"),
                  ],
                ),
                trailing: ElevatedButton(
                  onPressed: () => _showConsentDialog(baseballClass['program']!, isLecture: true),
                  child: Text("신청"),
                ),
              );
            },
  ...
  // 개인정보 동의 및 폼 호출 다이얼로그 (강좌/장소 분리)
  void _showConsentDialog(String programName, {required bool isLecture}) {
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setState) => AlertDialog(
          ...
          content: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              Text("$programName ${(isLecture ? '강좌' : '장소')} 신청에 개인정보 동의가 필요합니다."),
              CheckboxListTile(
          ...
          actions: [
            TextButton(
              onPressed: isChecked ? () => _showFormDialog(programName, isLecture: isLecture) : null,
              child: Text("확인"),
            ),
            TextButton(
              onPressed: () => Navigator.of(context).pop(),
  ...
  // 강좌 신청 폼
  void _showFormDialog(String programName, {required bool isLecture}) {
    ...
    showDialog(
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setState) => AlertDialog(
          title: Text(isLecture ? "수강생 정보 입력" : "이용자 정보 입력"),
          ...
          actions: [
            TextButton(
              onPressed: () {
                ...
                _showConfirmationDialog(isLecture: isLecture);
              },
              child: Text("확인"),
            ),
            TextButton(
  ...
  // 신청 완료 확인 다이얼로그
  void _showConfirmationDialog({required bool isLecture}) {
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        title: Text(isLecture ? "강좌신청 완료" : "장소신청 완료"),
        content: Text(isLecture ? "강좌신청이 완료되었습니다." : "장소신청이 완료되었습니다."),
        actions: [
          TextButton(
            onPressed: () => Navigator.of(context).pop(),
            child: Text("확인"),
        ...
        children: [
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return Card(
                      ...
                      child: ListTile(
                        ...
                        leading: Icon(Icons.sports_baseball, color: Colors.red),
                        trailing: facility['rentalAvailable'] == '가능'
                            ? ElevatedButton(
                          onPressed: () => _showConsentDialog(facility['title'], isLecture: false),
                          child: Text("장소이용신청"),
                        )
                            : null,
                      ),
          ...
          Positioned(
            ...
            child: ElevatedButton(
              onPressed: _showBaseballClasses,
